...
const Comments = ({postId,fromId,fromPhotoProfileUrl,fromUsername,timestamp,comment,commentId,toId,navigation}) => {
               ...
               const sub = db.collection('posts')
                    ...
                    .then(docc => {
                        ...
                        if (show == 'like2') {
                            db.collection("posts")
                                ...
                                .then(doc2 => {
                                    if (doc2.data()) {
                                        ...
                                        db.collection("posts").doc(postId).collection("comments").doc(commentId).collection("likes").doc(user.uid).set({
                                            likes: 1,
                                            fromId:user.uid,
                                            timestamp:Date.now(),
                                            toId:toId,
                            ...
                            db.collection('posts').doc(postId).collection("comments").doc(commentId).collection('likes').doc(user.uid).delete().then(function () {
            ...
            useEffect(() => {
                db.collection('posts').doc(postId).collection("comments").doc(commentId).collection("likes").where("clicked","==",true)
               .onSnapshot(snapshot => (
               ...
               const reportComment = (event) => {
                ...
                if(!input.trim()){
                    ...
                    db.collection("posts").doc(postId).collection("comments").doc(commentId).collection("reports").add({
                        report: input,
                        read: false,
                        count:false,
                        postId:postId,
